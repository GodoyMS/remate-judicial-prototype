...
export function YapePaymentForm({ defaultAmount, currency = "PEN", value, onChange }: YapePaymentFormProps) {
    ...
    >
      ...
      <div className="overflow-hidden rounded-2xl border border-[#742284]/20 bg-gradient-to-r from-[#742284] to-[#9B4DB8] text-white shadow-lg shadow-[#742284]/15">
        <div className="flex flex-col sm:flex-row sm:items-stretch">
          ...
          <div className="flex min-w-0 sm:min-w-[140px] flex-col items-start sm:items-end justify-center border-t sm:border-t-0 sm:border-l border-white/15 bg-black/10 px-5 py-4">
      ...
      {/* Step progress */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2 rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            <InputOTPGroup className="w-full justify-center gap-1.5 sm:gap-2.5">
              {Array.from({ length: 6 }).map((_, i) => (
                <InputOTPSlot
                  key={i}
                  index={i}
                  className={cn(
                    "size-10 rounded-xl border-border/80 bg-muted/20 text-base font-semibold shadow-sm first:rounded-xl last:rounded-xl sm:size-12 sm:text-lg",
      ...
      <div className="flex items-start gap-2.5 rounded-xl border border-[#742284]/15 bg-[#742284]/5 px-4 py-3">
        ...
        <p className="text-[11px] leading-relaxed text-muted-foreground">
          ...
          <strong className="text-foreground">
            {currencySymbol} {payAmount.toLocaleString("es-PE", { minimumFractionDigits: 2 })}
          </strong>{" "}
