...
export function AdminNotificationsPopover() {
  ...
  const urgentCount = unreadNotifications.filter(
    (n) => n.priority === "high"
  ).length;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <Bell
            className={cn("size-4", unreadCount > 0 && "text-foreground")}
          />
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 flex size-4 items-center justify-center rounded-full bg-destructive text-[9px] font-bold text-destructive-foreground ring-2 ring-background">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {unreadCount > 0
                  ? `${unreadCount} pendientes${urgentCount > 0 ? ` · ${urgentCount} urgentes` : ""}`
                  : "Backoffice al día — sin pendientes"}
              </p>
            </div>
            {unreadCount > 0 && (
        ...
        >
          ...
          <TabsContent value="unread" className="mt-0">
            <NotificationList
              ...
              onToggle={toggleRead}
              emptyMessage="¡Excelente! No tienes pendientes"
              emptySub="Revisa la pestaña Todos para ver el historial"
            />
          </TabsContent>
        </Tabs>
...
function NotificationList({
  ...
  items: ReturnType<typeof useAdminNotifications>["notifications"];
